...
class Net(nn.Module):
    ...
    def __init__(self, student_n, exer_n, knowledge_n):
        ...
        # with open("/tic_disk/zhangyunfei/confidence_work1/15-fifteenth/junyi/junyi_diff.txt", "r") as f:  
        #     self.k_difficulty = f.read()
        # self.k_difficulty = eval(self.k_difficulty)

        # with open("/tic_disk/zhangyunfei/confidence_work1/15-fifteenth/junyi/junyi_disc.txt", "r") as f:  
        #     self.e_discrimination = f.read()
        # self.e_discrimination = eval(self.e_discrimination)
    ...
    def forward(self, stu_id, exer_id, kn_id, d_type):     
        if d_type == 'train':
            ...
            # k_difficulty = torch.ones((exer_id.shape[0], self.knowledge_dim))
            # e_discrimination = torch.ones((exer_id.shape[0], self.knowledge_dim))
            # for i in range(len(exer_id)):
            #     k_difficulty[i] =  torch.tensor(self.k_difficulty[exer_id[i]])
            #     e_discrimination[i] =  torch.tensor(self.e_discrimination[exer_id[i]])

            # k_difficulty = k_difficulty.to(self.device) 
            # e_discrimination = e_discrimination.to(self.device) * 10
